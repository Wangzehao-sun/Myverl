...
def get_ratio_stats(
    ...
    return all_stats
```// filepath: /home/zhwang/A100/LLM/Train/verl/verl/custom/new_dp_actor.py
// ...existing code...
                        if 'off_ratio_min_clip_frac' in ret_dict:
                            data['actor/off_ratio_min_clip_frac'] = ret_dict['off_ratio_min_clip_frac'].detach().item()
                        
                        # 添加对 on/off ratio 和 log_ratio 的统计
                        ratio_stats = get_ratio_stats(
                        ...
                        for key, val in ratio_stats.items():
                            data[f'actor/{key}'] = val.detach().item()
...
def get_ratio_stats(
    ...
    all_stats.update(_calculate_and_get_stats(off_ratio, off_mask, "off_ratio", is_log=False))
    all_stats.update(_calculate_and_get_stats(off_log_ratio, off_mask, "off_log_ratio", is_log=