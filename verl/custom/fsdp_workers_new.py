...
class NewActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        if rollout_name == "hf":
            ...
            from .new_vllm_rollout import NewvLLMRollout
            ...
            if vllm_mode == "customized":
                ...
                vllm_rollout_cls = NewvLLMRollout if self.config.rollout.mode == "sync" else vLLMAsyncRollout
                rollout = vllm_rollout_cls(model_path=local_path, 
                                           config=self.config.rollout, 
                                           tokenizer=self.tokenizer, 
                                           model_hf_config=self.actor_model_config, 
                                           device_mesh=rollout_device_mesh, 
                                           trust_remote_code=trust_remote_code, **lora_kwargs)
            else:
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            with Timer(name="update_policy", logger=None) as timer:
                metrics, tensors_dict_to_return = self.actor.update_policy(data=data)
            ...
            output = DataProto.from_dict(tensors=tensors_dict_to_return, meta_info={"metrics": metrics})
            # 如果需要将 log_prob_shard 传回 trainer，可以这样做：
...
class AsyncActorRolloutRefWorker(NewActorRolloutRefWorker):
    def _build_rollout(self, trust_remote_code=False):
