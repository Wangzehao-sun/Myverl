...
val_files="['$test1_path', '$test2_path', '$test3_path']"
...
python -m verl.trainer.main_ppo_new \
    ...
    +se_model.enable=True \
    +se.target='standard' \
    +actor_rollout_se.model.path=$MODEL_PATH \
    +actor_rollout_se.rollout.name=sglang \
    ...
    +actor_rollout_ref.actor.policy_loss.off_policy_reshape="cispo" \
